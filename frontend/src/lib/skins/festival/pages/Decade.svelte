...
<div class="relative overflow-hidden rounded-2xl p-6 text-center text-white shadow-lg" style="background-image: linear-gradient(to bottom right, var(--fest-banner-from), var(--fest-banner-to));">
  ...
  <div class="relative">
    ...
    <p class="mx-auto mt-3 max-w-xl text-sm text-white/90">{T('intro', 'decade_intro')}</p>
  </div>
</div>

<div class="{CARD} mt-4 p-5">
  <h3 class="text-base font-black text-[rgb(var(--accent))]">{T('origin_h', 'decade_origin_h')}</h3>
  <div class="mt-2 space-y-2 text-sm text-[rgb(var(--fest-ink))]">
    ...
    <p class="font-semibold text-[rgb(var(--fest-ink))]">{T('origin_p4', 'decade_origin_p4')}</p>
  </div>
</div>

<h3 class="mb-3 mt-4 px-1 text-sm font-extrabold text-[rgb(var(--accent))]">{T('journey_h', 'decade_journey')}</h3>
<div class="{CARD} p-5">
...
<div class="{CARD} mt-4 p-5 text-center">
  <p class="font-mono text-xs text-[rgb(var(--fest-ink)/0.7)]">{T('evolution_line', 'decade_evolution_line')}</p>
  <h3 class="mt-2 text-base font-black text-[rgb(var(--accent))]">{T('portal_h', 'decade_portal_h')}</h3>
  <p class="mt-1 text-sm text-[rgb(var(--fest-ink))]">{T('portal_p', 'decade_portal_p')}</p>
</div>
...
<div class="{CARD} mt-4 p-5">
  ...
  <div class="mt-4 rounded-xl bg-[rgb(var(--accent)/0.1)] p-4">
    <p class="text-xs font-extrabold uppercase text-[rgb(var(--accent))]">{T('totals_h', 'decade_totals_h', rangeVars)}</p>
    <p class="mt-1 text-lg font-black text-[rgb(var(--fest-ink))]">{T('total_amount', 'decade_total_amount', { amount: fmt(d.grandTotal) })}</p>
    <p class="text-sm font-bold text-[rgb(var(--fest-ink))]">{T('total_entries', 'decade_total_entries', { count: d.grandContributors })}</p>
    <p class="mt-2 text-xs text-[rgb(var(--fest-ink)/0.7)]">{T('total_clarify', 'decade_total_clarify', { count: d.grandContributors })}</p>
  </div>
</div>

<h3 class="mb-3 mt-4 px-1 text-sm font-extrabold text-[rgb(var(--accent))]">{T('timeline_h', 'decade_timeline_h')}</h3>
<div class="{CARD} p-5">
  <ol class="relative space-y-3 border-l-2 border-[rgb(var(--accent-2))] pl-5">
    {#each TIMELINE as [key, i18n], i}
      <li class="relative">
        ...
        <p class="text-sm font-black text-[rgb(var(--accent))]">{T(key + '_h', i18n + '_h')}</p>
        <p class="mt-0.5 text-sm text-[rgb(var(--fest-ink))]">{T(key + '_d', i18n + '_d')}</p>
      </li>
    {/each}
  </ol>
</div>
...
<div class="{CARD} mt-4 p-5">
  <h3 class="text-base font-black text-[rgb(var(--fest-ink))]">{T('think_h', 'decade_think_h')}</h3>
  <p class="mt-1 text-sm font-bold text-[rgb(var(--accent))]">{T('think_lead', 'decade_think_lead')}</p>
  <p class="mt-2 text-sm text-[rgb(var(--fest-ink))]">{T('think_p', 'decade_think_p')}</p>
  <div class="mt-4 grid gap-2.5 sm:grid-cols-3">
    {#each DECADE_MILESTONES as m}
      ...
      <div class="rounded-xl border border-[rgb(var(--accent-2)/0.5)] p-4">
        <p class="text-sm font-black text-[rgb(var(--accent))]">{T(msKey + '_h', m.headingKey)}</p>
        <p class="mt-0.5 text-xs text-[rgb(var(--fest-ink)/0.8)]">{T(msKey + '_d', m.bodyKey)}</p>
      </div>
    {/each}
  </div>
</div>
...
<p class="mt-5 text-center text-sm font-medium text-[rgb(var(--accent))]">{T('footer', 'decade_footer')}</p>
